...
export default function CheckoutPanel() {
    ...
    // The confirmed id we actually query with
    const [verifiedInput, setVerifiedInput] = useState<string>("");

    const ticketId = useMemo(() => verifiedInput.trim() || undefined, [verifiedInput]);
    ...
    // - if first time (ticketId !== id): just set state -> hook will fetch
    // - if same id again: use refetch() to force fetching
    function doFetchTicket(id: string) {
        const trimmed = id.trim();
        if (!trimmed) return;

        if (ticketId === trimmed) {
            refetchTicket(); // same key -> force refetch
        } else {
            setVerifiedInput(trimmed); // first-time or different id -> hook will fetch
        }
    }

    // Show toast once the query settles (works for both initial fetch and refetch)
    useEffect(() => {
        if (!ticketId) return;          // nothing requested yet
        if (isFetchingTicket) return;   // still loading

        if (isTicketError) {
            const msg = ticketError instanceof Error ? ticketError.message : "Failed to fetch ticket";
            toast(msg, "error");
            return;
        }
    ...
    }, [ticketId, isFetchingTicket, isTicketError, isTicketSuccess, ticket, ticketError]);

    // Verify subscription result toast (once per attempt)
    useEffect(() => {
        ...
        if (sub) {
            toast(
                sub.active ? `Subscription ${sub.id} is Active.` : `Subscription ${sub.id} is Inactive.`,
    ...
    const showSubscriberDetails = !!verifiedSubId && !subLoading && !subError && !!sub;

    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-900">
            <div className="w-[90%] mx-auto p-4">
                <div className={sub ? "grid gap-6 lg:grid-cols-2" : "max-w-[30rem] mx-auto"}>
                    {/* LEFT: Checkout card */}
                    ...
                    {sub && <div className="rounded-xl border bg-white p-4 shadow-sm">
                        <h3 className="mb-3 text-base font-semibold text-gray-900">Subscriber details</h3>
